...
class ProductFile(models.Model):
    ...
    def genetate_download_url(self):
        bucket = getattr(settings, 'AWS_STORAGE_BUCKET_NAME')
        region = getattr(settings, 'S3REDIRECT_REGION')
        accesss_key = gettatr(settings, "AWS_ACCESS_KEY_ID")
        secret_key = getattr(settings, "AWS_SECRET_ACCESS_KEY")
        if not secret_key or not accesss_key or not bucket or not region:
        ...
        aws_dl_object = AWSDownload(accesss_key, secret_key, bucket, region)
